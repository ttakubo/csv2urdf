...
tpl = env.get_template('template_gazebo.xacro')

#CSVファイル読み込み 引数指定がないときはデフォルトでrobot_param.csv
if len(args) > 1 :
  f = open(args[1], 'r', newline='')
else :
  f = open('robot_param2.csv', 'r', newline='')
...
input_f  = robot_name+".xacro"
output_f = robot_name+".urdf"

# 実行コマンド
cmd = "ros2 run xacro xacro %s > %s" % (input_f, output_f)
